...
// RemoteClusterWatcher manages AgentPools for trusted (remote) clusters. It
...
type RemoteClusterWatcher struct {
	cfg RemoteClusterWatcherConfig
...
// RemoteClusterWatcherConfig is a bundle of parameters used by a
// RemoteClusterWatcher.
type RemoteClusterWatcherConfig struct {
	...
	// KubeDialAddr is an address of a local kubernetes proxy.
...
// NewRemoteClusterWatcher creates a new unstarted watcher with the provided
// config. Call Run() to start the watcher.
func NewRemoteClusterWatcher(cfg RemoteClusterWatcherConfig) *RemoteClusterWatcher {
...
// Close cleans up all outbound tunnels and stops the watcher.
func (w *RemoteClusterWatcher) Close() error {
	w.mu.Lock()
	defer w.mu.Unlock()
...
// Run runs the watcher polling loop. Run is blocking, start it in a goroutine.
func (w *RemoteClusterWatcher) Run(ctx context.Context) {
	w.mu.Lock()
...
func (w *RemoteClusterWatcher) Sync(ctx context.Context) error {
	...
	for k := range wantClusters {
		...
		pool, err := NewAgentPool(ctx, AgentPoolConfig{
			...
			// RemoteClusterWatcher only runs on proxies.
		...
		if err != nil {
			errs = append(errs, trace.Wrap(err, "failed creating reverse tunnel pool for remote cluster %q at address %q: %v", k.cluster, k.addr, err))
			continue
		}
		go pool.Start()
		w.pools[k] = pool
...
func (w *RemoteClusterWatcher) Counts() map[string]int {
	w.mu.Lock()
	defer w.mu.Unlock()
